...
def download_experiment_assets(project_name, experiment_name):
    ...
    # Construct the experiment path and fetch the experiment
    experiment_path = f"{project_name}/{experiment_name}"
    logging.info(f"Attempting to fetch experiment: {experiment_path}")

    try:
        # Fetch the experiment with a retry mechanism to handle temporary connection issues
    ...
    # Get the asset list and check if it's empty
    ...
    # Download each asset and handle any exceptions that occur
    for asset in asset_list:
        ...
        try:
            # Download the asset with a retry mechanism to handle temporary connection issues
